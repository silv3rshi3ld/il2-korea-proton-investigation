...
| D17 | Add RADV `startup,fullsync` to the same D16 tool and descriptor gate | [prepared](evidence-d17-radv-fullsync-preparation.md) | Synchronize after every draw/dispatch and flush all caches while retaining the exact D16 descriptor observations | diagnostic only; broad/slow driver control, never a proposed launch option or fix |
