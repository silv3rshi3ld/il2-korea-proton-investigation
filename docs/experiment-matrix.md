...
| D18 | Add RADV `startup,nodcc` to the same D16 tool and descriptor gate | [prepared](evidence-d18-radv-nodcc-preparation.md) | Disable Delta Color Compression while retaining exact `t9`/`t10` mapping telemetry | diagnostic only; isolates image compression metadata, never a proposed launch option or fix |
