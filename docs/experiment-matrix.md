...
| D20 | Enable VKD3D-Proton's built-in RenderDoc integration and capture the first queue submission containing pixel shader `df0bd777fd1bb89d` | [built; deployment pending Steam exit](evidence-d20-renderdoc-preparation.md) | Inspect exact `t9`/`t10` values at an affected consumer without replacing shaders or changing rendering commands | diagnostic only; value-level discriminator selected by D15-D19 |
