...
| D15 | Gated post-count dispatch and all-resource legacy-barrier telemetry; commit `9c6a4338…` | [prepared](evidence-d15-light-list-sync-preparation.md) | Identify `ComputeLightsFirstRef`/`ComputeLightsIndices` at runtime and determine whether the separate light-index buffer has a dependency before pixel consumption | diagnostic only; no rendering mutation |
