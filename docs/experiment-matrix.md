...
| D19 | Add ACO `force-waitcnt` with normal DCC/cache behavior to the same D16 tool and descriptor gate | [prepared](evidence-d19-aco-force-waitcnt-preparation.md) | Force conservative wait-state emission inside compiled shaders while retaining exact binding telemetry | diagnostic only; last cheap compiler-hazard control before produced-value capture |
