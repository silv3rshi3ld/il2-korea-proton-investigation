...
| D05 | Gated normalization of only the observed internal one-texel BC3 baked-terrain border copies to a complete 4-texel physical block; commit `35bd875c…` | [built and prepared](evidence-d05-preparation.md); `D05-bc3-r1` awaiting runtime | Enable marker must appear; analyzer must validate every adjusted shape/footprint; compare magenta seams separately from whole-page loss in one matched high-altitude run | pending; concrete causal discriminator |
