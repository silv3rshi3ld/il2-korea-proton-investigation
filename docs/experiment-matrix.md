...
| U00 | Game build `24596901`; Proton-supplied VKD3D at `3dfc6f07…` | prepared; runtime pending | Determine whether the game update changed the established defect before reinstalling diagnostics | inconclusive |
| D01 | `VKD3D_IL2_RESOURCE_TRACE=1`; trace-only build at `d0b4421f…` | initial run invalidated before launch by game update; pending U00 | Determine whether the title calls reserved/tiled-resource APIs on the failing path | inconclusive |
