...
| D05b | Footprint-aware revision with candidate/rejection telemetry; commit `f6416c79…` | [built, not installed or run](evidence-d05b-preparation.md) | Every target candidate must be adjusted or have an explicit safety-rejection mask; compare seams and whole-page loss separately | paused; next causal discriminator |
