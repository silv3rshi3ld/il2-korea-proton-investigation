...
| D05 | Gated normalization of only the observed internal one-texel BC3 baked-terrain border copies to a complete 4-texel physical block | planned; [evidence](evidence-d02-bc3-border-copies.md) | Bounded adjustment counter must match the targeted regions; compare magenta seams separately from whole-page loss in one matched high-altitude run | pending; concrete causal discriminator |
