...
| D16 | Normal-path descriptor write/copy sidecar plus fixed `t9`/`t10` draw-time resolution; commit `274f6f8e…` | [prepared](evidence-d16-descriptor-trace-preparation.md) | Resolve the two pixel consumers to descriptor type, resource cookie/shape, view metadata, and buffer range without enabling descriptor QA | diagnostic only; no rendering mutation, CPU timing/memory overhead is not a performance result |
