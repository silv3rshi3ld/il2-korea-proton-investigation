...
| G16 | A screen-space or temporal reflection resource contains stale or incorrectly synchronized tiles. | The game binaries name current/previous SSR targets and multiple aircraft/cockpit reflection passes. D11 now traces application-supplied resource names and PIX labels without changing rendering. | If D11 identifies the path, correlate its resource cookies with bindings, barriers, and draws/dispatches; otherwise trace those operations by resource description. |
