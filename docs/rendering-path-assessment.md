...
1. **Descriptor contents, index, or lifetime common to both backends.** A
...
2. **Copy-to-sample or UAV dependency.** The resource may have correct bytes
...
4. **RADV handling of otherwise valid Vulkan.** This remains possible but
...
First run one unmodified current-upstream VKD3D-Proton control. Microsoft
Flight Simulator 2024 twice exposed `dxil-spirv` defects, including one which
`spirv-val` did not catch and was selected by grass/near-ground rendering. The
installed Proton revision contains those historical fixes, but current
upstream carries 36 newer translator commits, including control-flow
structurizer changes. A fix would select a source-range bisection; an unchanged
result would remove a broad “already fixed upstream” possibility.

If current upstream is unchanged, use VKD3D-Proton's existing GPU-assisted
descriptor QA build. It can report heap out-of-range access, descriptor-type
mismatch, and access to a destroyed resource with shader hash, instruction
number, heap cookie, resource/view cookie, and descriptor index.
...
why the MSFS host-import fallback is not selected for IL-2.
