...
VKD3D-Proton d3d12.dll/d3d12core.dll, and DXVK dxgi.dll. None of the six
controlled logs loads d3d11.dll.
...
Assessment
The altitude dependence and rectangular pages make terrain streaming,
residency, sparse/tiled-resource handling, or mip/LOD behavior the leading
hypothesis class. Current PROTON_LOG output contains no terrain resource IDs,
mip ranges, tile mappings, residency changes, or altitude, so the defective
layer (game, VKD3D-Proton, or RADV) is not isolated. No override or source patch
is proposed.
...
3dfc6f07d0953b1e8b41705275c2c59cc7374fc5, game build 24577563.
...
does not expose altitude, terrain resource IDs, tile mappings, residency, or
mip ranges, so this does not yet identify sparse resources or an LOD bug.
...
Current conclusion: no game override or general patch is justified. If further
testing is useful, the next high-value step is matched low/high-altitude capture
with descriptor buffers disabled, followed by filtered instrumentation of the
terrain resource creation, mip/tile/residency changes, descriptor association,
and the operation that makes each rectangular page visible.
```
