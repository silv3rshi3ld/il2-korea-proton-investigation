...
{
    uint32_t type_id, extract_id, composite_id, zero_id, mask_id, shift_id;
    uint32_t new_id, *input_words, *output_words;
    ...
    if (argc != 9)
    {
        fprintf(stderr, "Usage: %s INPUT.spv OUTPUT.spv TYPE_ID EXTRACT_ID "
                "COMPOSITE_ID ZERO_ID MASK_ID SHIFT_ID\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    mask_id = parse_id(argv[7], "mask constant ID");
    ...
    new_id = input_words[3];
    if (!new_id || new_id == UINT32_MAX)
    {
        fprintf(stderr, "Cannot allocate a fresh SPIR-V ID from bound %"PRIu32"\n",
                new_id);
        free(input_words);
        return EXIT_FAILURE;
    }
    ...
    output_words[insert_offset + 0] = (5u << 16) | SPV_OP_BITWISE_AND;
    output_words[insert_offset + 1] = type_id;
    output_words[insert_offset + 2] = new_id;
    output_words[insert_offset + 3] = extract_id;
    output_words[insert_offset + 4] = mask_id;
    memcpy(&output_words[insert_offset + 5], &input_words[insert_offset],
            (input_count - insert_offset) * sizeof(*output_words));
    output_words[3] = new_id + 1;
    ...
    printf("D28_PATCH input=%s output=%s extracted_id=%"PRIu32
            " filtered_id=%"PRIu32" mask_constant_id=%"PRIu32
            " zero_tests=%zu record_shifts=%zu\n",
            argv[1], argv[2], extract_id, new_id, mask_id,
            equal_matches, shift_matches);
    free(output_words);
    free(input_words);
    return EXIT_SUCCESS;
