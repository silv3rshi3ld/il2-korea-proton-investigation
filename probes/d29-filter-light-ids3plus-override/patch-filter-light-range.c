...
{
    uint32_t type_id, extract_id, composite_id, zero_id, threshold_id;
    uint32_t shift_id, bool_type_id, condition_id, filtered_id;
    uint32_t select_true_id, select_false_id;
    uint32_t *input_words, *output_words;
    size_t input_count, output_count, extract_offset, insert_offset;
    size_t offset, equal_matches = 0, shift_matches = 0;

    if (argc != 10 && argc != 11)
    {
        fprintf(stderr, "Usage: %s INPUT.spv OUTPUT.spv TYPE_ID EXTRACT_ID "
                "COMPOSITE_ID ZERO_ID THRESHOLD_ID SHIFT_ID BOOL_TYPE_ID "
                "[REPLACE_BELOW_ID]\n",
                argv[0]);
        return EXIT_FAILURE;
    }
    ...
    threshold_id = parse_id(argv[7], "threshold constant ID");
    shift_id = parse_id(argv[8], "light-record shift constant ID");
    bool_type_id = parse_id(argv[9], "boolean type ID");
    if (argc == 11)
    {
        select_true_id = parse_id(argv[10], "replacement constant ID");
        select_false_id = extract_id;
    }
    else
    {
        select_true_id = extract_id;
        select_false_id = zero_id;
    }
    ...
    output_words[insert_offset + 0] = (5u << 16) | SPV_OP_U_LESS_THAN;
    ...
    output_words[insert_offset + 4] = threshold_id;
    ...
    output_words[insert_offset + 9] = select_true_id;
    output_words[insert_offset + 10] = select_false_id;
    ...
    printf("RANGE_FILTER_PATCH input=%s output=%s extracted_id=%"PRIu32
            " condition_id=%"PRIu32" filtered_id=%"PRIu32
            " threshold_constant_id=%"PRIu32" select_true_id=%"PRIu32
            " select_false_id=%"PRIu32" zero_tests=%zu record_shifts=%zu\n",
            argv[1], argv[2], extract_id, condition_id, filtered_id,
            threshold_id, select_true_id, select_false_id,
            equal_matches, shift_matches);
    free(output_words);
    free(input_words);
    return EXIT_SUCCESS;
